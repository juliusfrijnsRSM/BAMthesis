...
for i, batch in enumerate(batches):
    ...
    post_abnormal_return_1week = (np.nanprod(1 + abnormal_return[:5]) - 1) * 100
    post_abnormal_return_1month = (np.nanprod(1 + abnormal_return[:21]) - 1) * 100
    post_abnormal_return_2months = (np.nanprod(1 + abnormal_return[:42]) - 1) * 100
    post_abnormal_return_3months = (np.nanprod(1 + abnormal_return[:63]) - 1) * 100
    ...
    # SIMULATION ONLY
    y_post_sim = batch_post["SimpleReturn"][1:] - batch_post["RF"][1:]
    X_post_sim = sm.add_constant(batch_post[["Mkt-RF", "SMB", "HML", "RMW", "CMA"]][1:])
    model_sim = sm.OLS(y_post_sim, X_post_sim).fit()
    
    pred_sim = model_sim.predict(X_post_sim)
    res_sim = y_post_sim - pred_sim
    abnormal_return_sim = res_sim.values
    
    abnormal_return_sim_2months = (np.nanprod(1 + abnormal_return_sim[:42]) - 1) * 100
    raw_return_sim_2months = (np.nanprod(1 + batch_post["SimpleReturn"][1:43]) - 1) * 100
    market_return_sim_2months = (np.nanprod(1 + batch_post["ReturnSP500"][1:43]) - 1) * 100
    ...
    records.append({
        ...
        # SIMULATION ONLY
        "PostAbnormalReturnSimulation": abnormal_return_sim_2months,
        "PostRawReturnSimulation": raw_return_sim_2months,
        "PostMarketReturnSimulation": market_return_sim_2months,
    })
...
# Export final df
df_final.to_csv("Block data/batches_postearn_FF5pre.csv")

# Save dtypes in json file as well
dtypes = df_final.dtypes.astype(str).replace("datetime64[ns]", "object")
dtypes.to_json("Block data/batches_postearn_FF5pre_dtypes.json")
