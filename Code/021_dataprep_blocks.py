...
# -*- coding: utf-8 -*-
"""
Created on Fri May  2 16:16:52 2025
...
for i, batch in enumerate(batches):
    ...
    excess_log_return = log_return - log_return_sp500
    #batch["ExcessLogReturn"] = excess_log_return
    
    ## Entity- and time-varying vars
    
    # Excess returns on the three days before the earnings call
    excess_return_1day_prior = excess_log_return[-2]
    excess_return_2day_prior = excess_log_return[-3]
    excess_return_3day_prior = excess_log_return[-4]
    
    # Cumulative excess returns leading up to earnings announcement
    # Used for to track momentum of stock leading up to earnings call
    three_month_excess_return = np.nansum(excess_log_return[:-1])
    one_month_excess_return = np.nansum(excess_log_return[-22:-1])
    one_week_excess_return = np.nansum(excess_log_return[-6:-1])
    three_day_excess_return = np.nansum(excess_log_return[-4:-1])
    ...
    records.append({
        ...
        # Index vars
        "LastDayIndexReturn": last_day_index_return,
        ...
        "ExcessReturn1DayPrior": excess_return_1day_prior,
        "ExcessReturn2DayPrior": excess_return_2day_prior,
        "ExcessReturn3DayPrior": excess_return_3day_prior,
        
        # Excess return summed (momentum) vars
        "ThreeMonthExcRetSummed": three_month_excess_return,
        "OneMonthExcRetSummed": one_month_excess_return,
        "OneWeekExcRetSummed": one_week_excess_return,
        "ThreeDayExcRetSummed": three_day_excess_return,
